...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (sendBroadcastBtn) {
        sendBroadcastBtn.addEventListener('click', async function () {
            ...
            if (!confirm('Are you sure you want to send this email to ALL registered players?')) {
                return;
            }
            ...
            try {
                if (typeof sendBroadcastToSheets === 'function') {
                    const result = await sendBroadcastToSheets(subject, message);
                    ...
                    if (result.success) {
                        ...
                        alert('❌ Failed to send broadcast: ' + (result.error || 'Unknown error'));
                    }
                } else {
                    alert('❌ Error: API function not found.');
                }
            } catch (error) {
